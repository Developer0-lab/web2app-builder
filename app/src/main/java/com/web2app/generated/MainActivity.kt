...
class MainActivity : Activity() {
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        when (requestCode) {
            ...
            REQUEST_LOCATION -> {
                ...
                if (origin != null && callback != null) callback.invoke(origin, grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED, false)
            }
        }
    }
    ...
    private inner class AppDownloadListener : DownloadListener {
        override fun onDownloadStart(url: String, userAgent: String, contentDisposition: String, mimetype: String, contentLength: Long) {
            if (!url.startsWith("http://") && !url.startsWith("https://")) {
                Toast.makeText(this@MainActivity, "This file type cannot be downloaded here.", Toast.LENGTH_SHORT).show(); return
            }
            try {
    ...
    private inner class AppWebViewClient : WebViewClient() {
        override fun onPageFinished(view: WebView, url: String?) { super.onPageFinished(view, url); splashHandler.postDelayed({ hideSplash() }, 150) }
        ...
        private fun launchWhatsApp(uri: Uri): Boolean = try {
            val whatsappIntent = Intent(Intent.ACTION_VIEW, uri).apply { setPackage("com.whatsapp") }
            try { startActivity(whatsappIntent) } catch (_: ActivityNotFoundException) { startActivity(Intent(Intent.ACTION_VIEW, uri)) }
            true
